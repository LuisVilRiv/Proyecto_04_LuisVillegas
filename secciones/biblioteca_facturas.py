...
class SeccionFacturas:
    ...
    def _build_ui(self):
        ...
        self._canvas.create_window((4, 4), window=self._inner, anchor="nw")
    ...
    def _renderizar_tarjeta(self, nombre: str):
        ...
        def kv(icon, lbl, val, vc=COLOR_TEXT):
            ...
            tk.Label(row, text=lbl, font=("Georgia", 10),
                     bg=COLOR_CARD, fg=COLOR_DIM, width=15, anchor="w"
                     ).pack(side="left")
            tk.Label(row, text=val or "—", font=("Georgia", 10, "bold"),
                     bg=COLOR_CARD, fg=vc, anchor="w", wraplength=370
                     ).pack(side="left", padx=(6, 10), pady=5)
        ...
        tk.Label(cab, text="COMPROBANTE DE PRÉSTAMO",
                 font=("Georgia", 9),
        ...
        if data:
            ...
            tk.Label(nb, text="PRÉSTAMO", font=("Georgia", 8),
            ...
            fb.pack(side="left")
            tk.Label(fb, text="FECHA DE EMISIÓN", font=("Georgia", 8),
                     bg=COLOR_CARD, fg=COLOR_DIM).pack(anchor="w")
            ...
            # Socio
            sec_head("👤", "DATOS DEL SOCIO")
            kv("🆔", "ID socio",  data["socio"]["id"])
            kv("✏",  "Nombre",    data["socio"]["nombre"], COLOR_GOLD)
            kv("✉",  "Correo",    data["socio"]["correo"])
            kv("📱", "Teléfono",  data["socio"]["telefono"])
            spacer()

            # Libro
            sep()
            sec_head("📖", "LIBRO PRESTADO")
            kv("🔢", "ISBN",   data["libro"]["isbn"])
            kv("📚", "Título", data["libro"]["titulo"])
            kv("✍",  "Autor",  data["libro"]["autor"])
            spacer()

            # Devolución
            sep()
            sec_head("🗓", "DEVOLUCIÓN", c=COLOR_ERROR)
            kv("⚠",  "Fecha límite", data["devolucion"]["fecha"], COLOR_ERROR)
            kv("⏱", "Plazo",         data["devolucion"]["plazo"])
            spacer(10)
        ...
        # Indicador PDF
        tk.Label(outer,
                 text="📄  Recibo en formato PDF — usa 'Abrir' para verlo o 'Descargar' para guardarlo.",
                 font=("Georgia", 9, "italic"),
    ...
    def _abrir_externo(self):
        ...
        try:
            if sys.platform.startswith("win"):
                os.startfile(self._ruta_activa)
            elif sys.platform == "darwin":
                subprocess.Popen(["open", self._ruta_activa])
            else:
                subprocess.Popen(["xdg-open", self._ruta_activa])
    ...
    def _descargar(self):
        ...
        destino = filedialog.asksaveasfilename(
            title=_t("descargar"),
            initialfile=os.path.basename(self._ruta_activa),
